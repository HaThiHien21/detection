...
app.config['MAIL_USERNAME'] = 'hahien24032001@gmail.com'
app.config['MAIL_PASSWORD'] = 'hien'
...
class employee(db.Model):
    ...
    users_id = db.Column(db.String(20), nullable=False)
    department_id = db.Column(db.String(20), nullable=False)
    service_id = db.Column(db.String(20), nullable=False)


    def __repr__(self) -> str:
        return f"{self.id} - {self.name} -  {self.email} - {self.hiringDate}  - {self.phone_number} - {self.users_id} - {self.department_id} - {self.service_id}" 
...
def send_mail(email, text):
    msg = Message('Successfully Registered', recipients=[email], sender = 'hahien24032001@gmail.com', body=text)
    mail_.send(msg)
...
def sendResetMail(mail,otp):
    msg = Message('Reset Password', recipients=[mail],sender='hahien24032001@gmail.com')
...
def add():
    ...
    if request.method == 'POST':
        ...
        try:
            invalid = 1
            new_employee = employee(id=id, name=name, email=mail, hiringDate=hiringDate,
                                        phone_number=phone_number, users_id=users_id,
...
def update():
    ...
    emp.hiringDate = request.form['hiringDate']
    emp.phone_number = request.form['phone_number']
    emp.users_id = request.form['users_id']
    emp.department_id = request.form['department_id']
    emp.service_id = request.form['service_id']
    db.session.commit()
    #update photo
    fileNm = id + '.jpg'
    try:
    ...
    df = pd.read_csv("static/records.csv")
    df.loc[(df["Id"] == id) & (df['Status'] == 'On Service'), ['Name','hiringDate','phone_number','users_id ','department_id','service_id']] = [emp.name,emp.hiringDate,emp.phone_number,emp.users_id,emp.department_id,emp.service_id]
    df.to_csv("static/records.csv", index=False)
    return redirect("/add")
    ...
    def markEntry(id):
        with open('static/records.csv', 'r+') as f:
            ...
            if (id not in idList):
                ...
                emp = employee.query.filter_by(id=id).first()
                f.writelines(
                    f'\n{id},{emp.name},{emp.department_id}, {dtime},{date},{"On Service"}')
...
def AttendanceSheet():
    ...
    fieldnames = ['Id', 'Name','mail','hiringDate','phone_number','users_id','department_id','service_id', 'Time', 'Date', 'Status']
...
def stats():
    ...
    db = pd.DataFrame(db)
    db = pd.DataFrame(data=list(map(lambda x: x.split(" - "), db[0])), columns=['Id', 'Name','Mail', 'hiringDate','Phone_number','users_id','department_id','service_id'])
    ...
    today_counts = pd.DataFrame(today.groupby(['department_id']).count()['Id'])
    db_counts = pd.DataFrame(db.groupby(['department_id']).count()['Id'])
    ...
    #department_id wise attendance in percentage&counts
    fig2 = []
    for d in db['department_id'].unique():
        present = len(today[today['department_id'] == d])
        fig2.append(px.pie(df, values=[present, len(db[db['department_id'] == d])-present], names=['Present', 'Absent'],  
         hole=.4, title=d + ' department_id', color_discrete_sequence=px.colors.qualitative.T10))

    #last 7 working days' attendance
    dates = df['Date'].unique()[-7:]
    df_last7 = df[df['Date'].isin(dates)]
    print(df_last7.columns)
    # Kiểm tra số lượng giá trị NaN trong cột "department_id" và "Date"
    nan_department_id = df_last7["department_id"].isnull().sum()
    nan_date = df_last7["Date"].isnull().sum()

    # Hiển thị số lượng giá trị NaN
    print(f"Số lượng giá trị NaN trong cột 'department_id': {nan_department_id}")
    print(f"Số lượng giá trị NaN trong cột 'Date': {nan_date}")
    fig3 = px.histogram(df_last7, x='Date', color="department_id", title='Date & department wise Attendance',  
    color_discrete_sequence=px.colors.qualitative.T10, template='presentation')

    #individual attendance percentage
    hiringDates = [datetime.date(datetime.strptime(d.strip(), '%d/%m/%Y')) for d in db['hiringDate']]
    ...
    return render_template('statsPage.html', JSON1=JSON1, JSON2=JSON2, JSON3=JSON3,depts=db['department_id'].unique(), 
